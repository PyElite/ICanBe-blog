...
def get_article_list(sort=None, num=None):
    ...
    if sort:
        if num:
            return Article.objects.order_by('-{}'.format(sort))[:num]
...
# 获取文章详情页下方的推荐阅读文章
@register.simple_tag
def get_category_article(article_obj):
    article_list = Article.objects.filter(tags__article=article_obj)
    if len(article_list) >= 12:
        n = 12
        i = random.randint(0, Article.objects.filter(tags__article=article_obj).count() - n)
        article_list = Article.objects.all()[i:i + n]
        article_4 = article_list[:4]
        article_8 = article_list[4:]
    else:
        article_list = get_article_list('loves', 12)
        article_4 = article_list[:4]
        article_8 = article_list[4:]
